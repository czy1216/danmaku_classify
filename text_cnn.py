...
import numpy as np
from danmu_utils import get_embedding, read_dataset_sentiment, get_index_to_embedding, batch_iter, div_train_and_test, sentence_to_indices
...
# 取数据
m, Tx, x, y = read_dataset_sentiment(file_path)
...
# 生成训练集与测试集
x_train_indices, x_dev_indices, y_train_oh, y_dev_oh = div_train_and_test(x_indices, y, num_classes=2, propertion=0.8)
...
def train(x_train, y_train, x_dev, y_dev):
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "model/text_cnn_sentiment", timestamp))
            print("Writing to {}\n".format(out_dir))
