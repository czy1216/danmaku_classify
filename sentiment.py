...
from danmu_utils import read_dataset_sentiment_three, div_train_and_test, get_embedding, softmax, sentence_to_indices, read_dataset_sentiment
...
# 取出数据,标注只取情感
m, Tx, x, y_sentiment = read_dataset_sentiment(file_path)
# 取出词向量表
...
#一个时间序列的attention,以张量为单位思考
#输入张量经过参数W映射到不同的空间，起到不同的效果，训练W，如（attention空间，情感空间，句子类型空间）
def one_step_attention(a):
    w = Dense(16, activation='tanh')(a)
    w = Dense(1, activation='relu')(w)
    w = Activation(softmax)(w)
    context = Dot(axes=1)([w, a])
    return context
...
def sentiment_model(Tx, input_shape, voc_embedding, word_to_index):
    ...
    X = LSTM(128, return_sequences = False)(context)
    X = Dropout(0.5)(X)
...
def train(m, Tx, x, y_sentiment, voc_embedding, word_to_index):
    ...
    #取测试集与训练集
    x_train_indices, x_test_indices, y_train_oh, y_test_oh = div_train_and_test(x_indices, y_sentiment, num_classes=2, propertion=0.8)
